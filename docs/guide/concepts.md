...
NidamJS is designed as a modular, event-driven system that overlays a desktop environment onto a standard web page.
Understanding its core architecture will help you build robust applications.

## Architecture

At a high level, NidamJS consists of a central **Core** that manages state and communication, and several distinct \*
\*Modules\*\* that handle specific UI features.

### The Core

The Core is the brain of NidamJS. It initializes the environment, manages global configuration, and serves as the
central hub for:

- **State Management:** Tracking open windows, active tasks, and user preferences.
- **Event Bus:** facilitating communication between disparate components (e.g., clicking a taskbar item to minimize a
  window).
...
NidamJS uses a reactive state model. When you open a window, the Core updates its internal registry. This change
automatically triggers updates in:
...
Interaction in NidamJS is heavily event-driven. Components emit events that you can listen to, allowing for loose
coupling between your app logic and the desktop UI.
...
One of the most complex aspects of a web desktop is managing the stacking order of elements. NidamJS handles this
automatically using a dynamic z-index system.
...
When a user clicks a window, NidamJS recalculates the stack, pushing the clicked window to the front while maintaining
the relative order of others.
...
This architecture allows NidamJS to coexist with other frontend frameworks. You can render a React app inside a NidamJS
window, or have an Angular app managing the NidamJS instance.

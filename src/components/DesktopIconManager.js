...
export default class DesktopIconManager extends BaseManager {
        ...
        try {
            ...
            icons.forEach(icon => {
                ...
                if (id && savedIcon && Array.isArray(savedIcon.classes)) {
                    const existingClasses = Array.from(icon.classList).filter(c =>
    ...
    _bindEvents() {
        this._delegator.on("mousedown", ".desktop-icon", this._handleDragStart.bind(this));
    }
    ...
    _handleDragStart(e, target) {
        ...
        const gridClasses = Array.from(icon.classList).filter(c =>
            c.startsWith('col-start-') ||
            c.startsWith('row-start-') ||
            c.startsWith('col-end-')
        );
    ...
    _handleDragMove(e) {
        ...
        const { element, startX, startY, initialLeft, initialTop, containerRect } = this._dragState;
    ...
    _snapToGrid(element) {
        ...
        const collidingElement = Array.from(this._root.querySelectorAll('.desktop-icon')).find(icon =>
            icon !== element &&
        ...
        if (collidingElement) {
            ...
            if (this._dragState && this._dragState.originalClasses) {
                const currentClassesToRemove = Array.from(element.classList).filter(c =>
                    c.startsWith('col-start-') ||
                    c.startsWith('row-start-') ||
                    c.startsWith('col-end-')
                );
        ...
        const classesToRemove = Array.from(element.classList).filter(c =>
            c.startsWith('col-start-') ||
            c.startsWith('row-start-') ||
            c.startsWith('col-end-')
        );
